...
export default class ScreenFade {
    static async fadeIn(duration: number): Promise<void> {
        return new Promise<void>(async (resolve, reject) => {
            if (!native.isScreenFadedOut() || native.isScreenFadingIn() || native.isScreenFadingOut()) return resolve();
    ...
    static async fadeOut(duration: number): Promise<void> {
        return new Promise<void>(async (resolve, reject) => {
            if (!native.isScreenFadedIn() || native.isScreenFadingIn() || native.isScreenFadingOut()) return resolve();
